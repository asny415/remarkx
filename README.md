...
- 不付官方 API 费用：用 `twikit` 以你的**小号**会话抓取（等同网页行为，非官方，
  只用小号）。
- **按需抓取，不定时轮询**：relay 只有在设备请求首页且数据超过 `poll_seconds`
  秒未更新时才抓一次 X；翻页只读缓存。一次阅读会话通常只触发 1 次请求，
  打开阅读器不勤就不产生任何 X 请求。
...
| `poll_seconds` | 数据缓存有效期（秒），默认 300。仅当设备请求首页且数据超过该时长未更新时才抓一次 X；调大=更省请求、内容更新更慢 |
| `poll_count` | 每次拉取条数 |
...
# 首次运行：无会话文件 -> 自动从浏览器导入 -> 开始服务（按需抓取）
```

**会话过期怎么办**：设备请求首页时 relay 发现失效会**自动重新从浏览器导入**；
若仍失败，说明浏览器里的登录态也过期了——在浏览器里打开 x.com 刷新
一下页面即可，下次请求首页自动恢复，全程不用手动干预。
...
首次打开（或数据超过 `poll_seconds` 秒未更新）时，中转站才去 X 抓一次
（约 10~20 秒，主要是下载图片），期间屏幕显示"正在从 X 抓取最新内容…"
加载页，可点取消。之后翻页全部读缓存，秒开。
...
| 状态页显示"尚未登录" | 在**浏览器**登录 x.com，然后在阅读器里刷新首页（自动导入）；或跑 `login` |
| 状态页显示"登录态失效" | 阅读器刷新首页时 relay 会自动从浏览器重新导入；若仍失败，在浏览器打开 x.com 刷一下页面再刷新（数据保留） |
...
- **频率**：已改为按需抓取（不定时轮询），只有设备请求首页且数据过期才抓一次；
  若 X 仍报 429，把 `poll_seconds` 调大（600+）降低抓取频率。

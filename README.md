...
```
长按顶部中央(≥0.7s, 位移≤40px, 仅手指触摸/笔迹免疫)
...
- **全内存**：不做本地持久化，进程内存里只保留当前阅读内容——刷新即整体
  重建为本次抓到的批（`poll_count` 条），翻到书尾按 cursor 续抓更早内容
  往后追加，API 与渲染全部直接读内存
- **流式双栏排版**：卡片按原子拆分（头部/图片不可拆、正文逐行可拆），
  跨栏跨页自动接续（续排块带「┆续」标记），单页填满不半截
- **转推/引用卡片**：纯转推显示「转发了」+ 竖线引用块（原帖作者/内容/图）；
  引用推文展示转发者评论 + 被引用原帖；卡片底部带 转/赞/评/阅 互动数

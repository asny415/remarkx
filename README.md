...
- **设备端直连 X**：带 Cookie 的 GraphQL 请求；启动时先拉取置顶文件夹
  （`PinnedTimelines`）显示**标签选择层**，点选哪个标签就只抓哪个标签的时间线
  （"为你推荐"=`HomeTimeline`、"正在关注"=`HomeLatestTimeline`、文件夹=
  `ListLatestTweetsTimeline`，接口与参数同官方网页版）；网页端内嵌的 **Grok 译文**
  优先显示，无译文时显示原文。
...
- **时钟校准**：X 服务器 NTP 授时，每次 API 响应的 HTTP `Date` 头即精确服务器
  时刻。App 用发送/接收时刻取中点（SNTP 式）估算本地时钟偏移；漂移 >2s 且以
  root 运行时直接修正设备系统时钟（`clock_settime`）——右上角常驻时钟、帖子
  时间"今年"判断、收藏时间戳、Telegram 退避全部随之准确；无权限修正时 App
  内部按偏移自动补偿显示。显示时区不受影响（仍由 config.json `timezone` 决定）
...
- **标签选择**：启动先显示标签列表（"为你推荐"、"正在关注" + 置顶文件夹，
  顺序同网页版），点选后只抓该标签的时间线；上次选过的标签标"上次使用"
  （每次启动仍显示选择层）；文件夹列表抓取失败时回退只显示两个固定标签
- **实时时间线**：选中标签即抓取，翻到书尾自动续抓更早内容（无后台定时抓取）
...
- **时钟自动校准**：启动后首次网络请求返回即对时；设备时钟漂移 >2s 时直接
  修正系统时钟（root），此后每次抓取响应都会重估，会话期间保持准确
...
2. **选择标签**：启动后显示标签列表（"为你推荐"、"正在关注" + 置顶文件夹，
   顺序同网页版），点按要阅读的标签开始抓取（上次选过的标"上次使用"）。
3. **翻页**：左右滑动翻页；底部从下往上滑 = 刷新当前标签时间线；
   顶部从上往下滑 = 退出回到原生界面。
4. **看图**：手指点按任意图片 → 全屏查看，左右滑切换多图，点按/下滑关闭。
5. **看详情**：点按任意卡片 → 看主帖全文与按热度排序的回复（回复逐页自动加载）；
   左右滑翻页，底部上滑加载更多，点某条回复钻入它的详情页，顶部下滑返回。
6. **批注收藏**：直接用笔在帖子上书写；离开该页时自动判定归属帖子（一笔划到
   几条帖子就收藏几条，一页可收藏多条）并：
   - 每帖渲染"帖+笔迹"图 + 完整链接存到 `book/`（`favs.json`）；
   - 若配置了 Telegram，后台推送图片（说明 = 帖子链接），成功即删除本地帖图。
7. **休眠**：短按电源键挂起（进度已保存）；5 分钟无操作自动休眠。

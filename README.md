...
- 你在某条帖子上用笔写下的任何批注，都会按笔迹的起始位置自动判定它属于哪条帖子；
- 单独渲染一张"该帖子 + 你的笔迹"的图片（帖子跨页/跨栏拆分时自动拼合，笔迹保持相对位置）；
...
- **写字即收藏**：笔迹起始位置 → 命中当前页的帖子块 → `renderFavorite` 把该帖在
  整本 feed 里跨页/跨栏的各个拆分块按阅读顺序拼成一张图，并**按块原位叠加笔迹**，
  宽度自动裁剪到帖子实际占位（不留半版空白）。
...
- **写字即收藏**：按笔迹起始位置锁定帖子，渲染"帖+笔迹"图存本地
...
4. **看全文**：点卡片上的"显示全文"按钮 → 全屏单栏阅读，左右滑翻页。
5. **批注收藏**：直接用笔在帖子上书写；离开该页时自动判定归属帖子并：
   - 渲染"帖+笔迹"图 + 完整链接存到 `book/`（`favs.json`）；
...
│   ├── xclient.*         X 直连客户端：GraphQL 抓取 + 媒体懒加载
│   ├── renderer.*        整页渲染器：双栏排版、全文页、收藏帖图(renderFavorite)
│   ├── pagestore.*       页面状态机：翻页、笔迹、收藏索引、Telegram 联动
